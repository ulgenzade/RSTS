...
namespace RestoranOtomasyonu
{
    public class VeritabaniIslemleri
    {
        ...
        #region Sipariş Oluşturma ve Onaylama Metodu
        public bool SiparisOlusturVeOnayla(int masaID, int kullaniciID, List<SepetUrunDto> sepetUrunleri)
        {
            // Eğer sepet boşsa, işlem yapmaya gerek yok.
            ...
            {
                baglanti.Open();
                // Transaction başlat: Bu, "ya hep ya hiç" prensibidir.
                // Eğer adımlardan birinde hata olursa, hiçbir değişiklik veritabanına kaydedilmez.
                using (MySqlTransaction transaction = baglanti.BeginTransaction())
                {
                    try
                    {
                        // --- ADIM 1: Ana Sipariş Kaydını Oluştur ---
                        ...
                        using (MySqlCommand siparisEkleKomut = new MySqlCommand(siparisEkleSorgu, baglanti, transaction))
                        {
                        ...
                        // --- ADIM 2: Sepetteki Her Ürünü Sipariş Detaylarına Ekle ---
                        decimal toplamTutar = 0;
                        string siparisDetayEkleSorgu = "INSERT INTO SiparisDetaylari (SiparisID, UrunID, Adet, BirimFiyat) VALUES (@siparisID, @urunID, @adet, @birimFiyat);";

                        foreach (var urun in sepetUrunleri)
                        {
                            using (MySqlCommand detayEkleKomut = new MySqlCommand(siparisDetayEkleSorgu, baglanti, transaction))
                            {
                                detayEkleKomut.Parameters.AddWithValue("@siparisID", yeniSiparisID);
                                detayEkleKomut.Parameters.AddWithValue("@urunID", urun.UrunID);
                                detayEkleKomut.Parameters.AddWithValue("@adet", urun.Adet);
                                detayEkleKomut.Parameters.AddWithValue("@birimFiyat", urun.BirimFiyat);
                                detayEkleKomut.ExecuteNonQuery();
                            }
                            // Toplam tutarı hesapla
                            toplamTutar += urun.Adet * urun.BirimFiyat;
                        }

                        // --- ADIM 3: Ana Sipariş Kaydının Toplam Tutarını Güncelle ---
                        string toplamTutarGuncelleSorgu = "UPDATE Siparisler SET ToplamTutar = @toplamTutar WHERE SiparisID = @siparisID;";
                        using (MySqlCommand tutarGuncelleKomut = new MySqlCommand(toplamTutarGuncelleSorgu, baglanti, transaction))
                        {
                        ...
                        // --- ADIM 4: Masanın Durumunu 'Dolu' Olarak Güncelle ---
                        string masaDurumGuncelleSorgu = "UPDATE Masalar SET Durum = 'Dolu' WHERE MasaID = @masaID;";
                        using (MySqlCommand masaGuncelleKomut = new MySqlCommand(masaDurumGuncelleSorgu, baglanti, transaction))
                        {
                            masaGuncelleKomut.Parameters.AddWithValue("@masaID", masaID);
                            masaGuncelleKomut.ExecuteNonQuery();
                        }

                        // Her şey yolunda gittiyse, tüm değişiklikleri veritabanına kalıcı olarak işle.
                        transaction.Commit();
                        return true;
                    }
                    catch (Exception ex)
                    {
                        // Herhangi bir adımda hata oluşursa, yapılan tüm değişiklikleri geri al.
                        transaction.Rollback();
                        System.Diagnostics.Debug.WriteLine("Sipariş onayı sırasında hata: " + ex.Message);
                        return false;
                    }
                }
            }
        ...
        #region Aktif Siparis ID Getirme Metodu
        /// <summary>
